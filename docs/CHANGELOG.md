...
- Added a [Security Policy](SECURITY.md), [developer setup guide](dev/setup.md), and [architecture overview](dev/architecture.md) covering reporting, environment separation, secrets, data ownership, access control, and local installation.
...
- Expanded the README with current workspace architecture, collaborator installation, environment setup, database seeding, tests, Worker commands, and safety boundaries.
- Added a staff [Client records how-to](how-to/clients.md) for client list/detail use and admin merge review.
- Added a current [Work Orders staff guide](how-to/work-orders.md) and updated staff navigation documentation for Quote Movement and Work Orders.
