...
describe("Quote Movement detail route shell", () => {
  ...
  it("opens a protected detail shell from the cached record identity", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Q260101" }),
    ).toBeInTheDocument();
  ...
  it("presents material summary sections without supporting evidence links", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: "View supporting evidence" }),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByRole("heading", { name: "Activity" }).closest("details"),
  ...
  it("keeps the last valid detail summary visible when refresh is stale and summarisation failed", async () => {
    ...
    expect(
      screen.queryByText(/job-[0-9a-f]|postgres:\/\//i),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows a safe first-summary failure while no prior summary exists", async () => {
    ...
    expect(
      screen.getByText(
        "Not yet summarised. What Matters Now updates automatically after meaningful source activity.",
      ),
    ).toBeVisible();
